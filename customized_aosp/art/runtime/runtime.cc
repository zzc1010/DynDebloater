...
bool Runtime::MYmatch_target_method(std::string& s){
    ...
    if(!instance_-> getShouldReadSchema()){
      return false;
    }

    if(instance_ -> getLoadingSchema()){ 
    ...
    if(!instance_->getLoadedSchema()&&instance_->getTarget_method_string_vector().empty()){
      ...
      if(should_read&&instance_->getShouldReadSchema()){
      ...
      // else{
      //   instance_->setShouldReadSchema(false,package_name);
      // }
      // else{
